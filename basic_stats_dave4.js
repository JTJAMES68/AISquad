...
export default class DiscordStats extends DiscordBasePlugin {
    ...
    async onMessage(message) {
        ...
        Logger.verbose('DiscordStats', 1, `Sent message in ${message.channel.id}`);
        if (message.channel.id != this.options.channelID) return;
        Logger.verbose('DiscordStats', 1, `Used discord command ${cmd}`);
        switch (cmd) {
            case "link":
                {
